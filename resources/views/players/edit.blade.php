...
<head>
    <meta charset="utf-8">
    <title> ویکسانا-ویرایش پلیر </title>
        ...
        <div class="container-fluid">
            <div class="layout-specing">
                <div class="d-md-flex justify-content-between">
                    <h5 class="mb-0"> ویرایش پلیر</h5>

                    <nav aria-label="breadcrumb" class="d-inline-block mt-4 mt-sm-0">
                        <ul class="breadcrumb bg-transparent rounded mb-0 p-0">
                            <li class="breadcrumb-item"><a href="index.html">ویکسانا</a></li>
                            <li class="breadcrumb-item active" aria-current="page">پلیر</li>
                        </ul>
                    </nav>
                </div>
                ...
                <div class="col-6 mt-3">
                    ...
                    <form action="{{route('player.update',$p->id)}}" method="post">
                        @csrf
                        @method('put')
                        <div class="mb-3">
                            <label for="exampleInput" class="form-label">نام پلیر</label>
                            <input type="text" class="form-control" id="exampleInput" value="{{$p->player_name}}" name="player_name" required>
                        </div>
                        <div class="mb-3">
                            <label for="exampleInput7" class="form-label">نام کاربری</label>
                            <input type="text" class="form-control" id="exampleInput7" value="{{$p->user_name}}" name="user_name" required>
                        </div>
                        <div class="mb-3">
                            <label for="exampleInput9" class="form-label">رمز عبور</label>
                            <input type="password" class="form-control" id="exampleInput9" value="{{$p->password}}" name="password" required>
                        </div>
                        <div class="mb-3">
                            <label for="exampleInputPassword1" class="form-label">ریلیم</label>
                            <select class="form-select" aria-label="Default select example" name="relam_id">
                                @foreach($relam as $r)
                                    <option value="{{$r->id}}" @if ($r->id == $p->relam_id) {{ 'selected' }} @endif>{{$r->relam_name}}</option>
                                @endforeach
                            </select>
                        </div>
