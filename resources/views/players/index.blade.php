            ...
            <div class="layout-specing">
                ...
                <div class="col-10 mt-4">
                    ...
                    <table class="table table-striped">
                        ...
                        <tr>
                            <th scope="col">#</th>
                            <th scope="col">نام پلیر</th>
                            <th scope="col">نام کاربری</th>
                            <th scope="col">ریلیم پلیر</th>
                            <th scope="col">عملیات</th>
                        </tr>
                        </thead>
                        <tbody>
                        @foreach($player as $key=> $r)
                            <tr>
                                <th scope="row">{{++$key}}</th>
                                <td>{{$r->player_name}}</td>
                                <td>{{$r->user_name}}</td>
                                <td><span class="badge rounded-pill bg-info text-white">{{$r->relam->relam_name}}</span></td>
                                <td>
                                    <form method="post" action="{{route('player.destroy',$r->id)}}">
                                        @csrf
                                        @method('delete')
                                        <a href="{{route('player.edit',$r->id)}}" class="btn btn-info" title="ویرایش"><i class="uil uil-pen text-white"></i> </a>
